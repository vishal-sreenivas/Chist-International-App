...
class StudentDashboardActivity : AppCompatActivity() {
    ...
    private fun setupCardClickListeners() {
        ...
        // Campus View
        findViewById<MaterialCardView>(R.id.campusViewCard).setOnClickListener {
            try {
                val url = "https://sites.google.com/view/christinternational?usp=sharing"
                val intent = Intent(Intent.ACTION_VIEW)
                intent.data = Uri.parse(url)
                startActivity(intent)
            } catch (e: Exception) {
                Toast.makeText(this, "Unable to open link: ${e.message}", Toast.LENGTH_LONG).show()
            }
        }
