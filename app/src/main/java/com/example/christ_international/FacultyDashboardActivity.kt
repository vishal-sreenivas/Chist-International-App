...
class FacultyDashboardActivity : AppCompatActivity() {
    ...
    private fun setupModuleClickListeners() {
        ...
        findViewById<MaterialCardView>(R.id.cardInternationalStudents)?.setOnClickListener {
            try {
                val intent = Intent(this, InternationalStudentsActivity::class.java)
                startActivity(intent)
            } catch (e: Exception) {
                e.printStackTrace()
                Toast.makeText(this, "Error: Unable to open International Students. Please try again.", Toast.LENGTH_LONG).show()
            }
        }
        ...
        // Alumni
        findViewById<MaterialCardView>(R.id.cardAlumni)?.setOnClickListener {
            showFeatureUnderDevelopment("Alumni Details")
        }
        ...
        findViewById<MaterialCardView>(R.id.cardAppointments)?.setOnClickListener {
            try {
                val intent = Intent(this, FacultyAppointmentsActivity::class.java).apply {
                    putExtra("faculty_emp_id", facultyEmpId)
                }
                startActivity(intent)
            } catch (e: Exception) {
                e.printStackTrace()
                Toast.makeText(this, "Error opening appointments: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
